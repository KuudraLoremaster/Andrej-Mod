...
public class ModArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, MobEffectInstance> MATERIAL_TO_EFFECT_MAP =
            (new ImmutableMap.Builder<ArmorMaterial, MobEffectInstance>())
                    .put(ModArmorMaterials.SAPPHIRE, new MobEffectInstance(MobEffects.NIGHT_VISION, 200, 1,
                            false,false, true))
                    .put(ModArmorMaterials.KUUDRA, new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 200, 1,
                            false,false, true))
                    .build();
    ...
    int prevWeight = AndrejMod.weight;
    @Override
        ...
        if (!level.isClientSide()) {
            if (hasFullSuitOfArmorOn(player)) {
                ...
                if (prevWeight != AndrejMod.weight) {
                    int difference = Math.abs(AndrejMod.weight - prevWeight);
                    AndrejMod.weight += difference;
                    prevWeight = AndrejMod.weight;
                }
            }
        }
    ...
    boolean isFat = false;
    public void kuudraArmor(Player player) {
        if (hasFullSuitOfArmorOn(player)) {
    ...
    public void addStatusEffectForMaterial(Player player, ArmorMaterial mapArmorMaterial,
